...
final class SQLiteTaskDataSource
{
    ...
    private func makeTask(from statement: OpaquePointer?) -> AcademicTask?
    {
        ...
        let microsoftEventId = sqlite3_column_text(statement, 8).map { String(cString: $0) }
        ...
        return AcademicTask(
            ...
            dueDate: dateFormatter.date(from: String(cString: dueDateText)) ?? Date(),
            priority: TaskPriority(rawValue: String(cString: priorityText)) ?? .medium,
            status: TaskStatus(rawValue: String(cString: statusText)) ?? .pending,
            type: TaskType(rawValue: String(cString: typeText)) ?? .task,
            microsoftEventId: microsoftEventId,
            isAddedToCalendar: sqlite3_column_int(statement, 9) == 1,
            calendarSyncStatus: CalendarSyncStatus(rawValue: String(cString: calendarSyncStatusText)) ?? .notAdded,
